...
class App extends React.Component {
  ...
  updatedCat = async (catToUpdate) => {
    try {
      let url = `${SERVER}/cats/${catToUpdate._id}`;
      let updatedCatObj = await axios.put(url, catToUpdate);
      
      // find the cat we updated in state, and replace it with the data we got back from the database
      let updateCatsArray = this.state.cats.map(cat => {
        return cat._id === catToUpdate._id
          ? updatedCatObj.data
          : cat;
      });
      this.setState({
        cats: updateCatsArray
      });
    } catch (err) {
      console.log('We have an error: ', err.response.data);
    }
